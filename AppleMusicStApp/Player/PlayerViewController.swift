...
class PlayerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // seconds: 기준시간, prefferd: 몇개로 분할하냐
        let unit = CMTime(seconds: 1, preferredTimescale: 10) // >>>0.1 초
        timeObserver = simplePlayer.addPeriodicTimeObserver(forInterval: unit, queue: DispatchQueue.main, using: { time in
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        // TODO: 뷰나갈때 처리 > 심플플레이어
        simplePlayer.pause()
        simplePlayer.replaceCurrentTime(with: nil)
    ...
    @IBAction func slideTouch(_ sender: Any) {
        print("adfasdfasf")
    }
...
extension PlayerViewController {
    ...
    func updatePlayButton() {
        ...
        if simplePlayer.isPlaying {
            let image = UIImage(systemName: "pause.fill", withConfiguration:  UIImage.SymbolConfiguration(pointSize: 40))
